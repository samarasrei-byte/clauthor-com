...
export function useMonix() {
  const [messages, setMessages] = useState<MonixMessage[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [isStreaming, setIsStreaming] = useState(false);
  const [config, setConfig] = useState<MonixConfig>(() => {
    const saved = localStorage.getItem("monix_config");
    return saved ? { ...DEFAULT_CONFIG, ...JSON.parse(saved) } : DEFAULT_CONFIG;
  });
  const abortRef = useRef<AbortController | null>(null);

  useEffect(() => {
    localStorage.setItem("monix_config", JSON.stringify(config));
  }, [config]);

  const updateConfig = useCallback((partial: Partial<MonixConfig>) => {
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    const userMsg: MonixMessage = { role: "user", content, timestamp: new Date() };
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/monix-chat`,
        {
          method: "POST",
          headers: {

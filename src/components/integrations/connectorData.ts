...
export const connectors: ConnectorData[] = [
  ...
  },
  {
    icon: Database,
    name: "Zapier",
    shortDesc: "Conecte com +6000 apps via webhook.",
    longDesc: "Cole a URL do webhook 'Catch Hook' de um Zap. Seus agentes disparam eventos (novo lead, e-mail respondido, tarefa criada) e o Zapier roteia pra qualquer um dos +6000 apps suportados (Slack, HubSpot, Gmail, Sheets, Notion, Salesforce, Airtable…). Ideal quando não temos conector nativo.",
    category: "Automação",
    status: "available",
    apiStatus: "live",
    ...
    fields: [
      { key: "webhook_url", label: "URL do Catch Hook", placeholder: "https://hooks.zapier.com/hooks/catch/…", required: true },
      { key: "secret", label: "Secret (opcional)", placeholder: "Assinatura HMAC pra validar origem", type: "password" },
    ],
    tools: ["trigger-zap", "send-event", "send-lead", "send-notification"],
  },
  {
    icon: Database,
    name: "n8n",
    shortDesc: "Automação self-hosted ou cloud via webhook.",
    longDesc: "Cole a URL do nó 'Webhook' do seu n8n (cloud ou self-hosted). Seus agentes disparam JSON e o n8n roteia pra qualquer sistema — inclusive APIs internas atrás de VPN, bancos de dados e ferramentas que Zapier não suporta.",
    category: "Automação",
    status: "available",
    apiStatus: "live",
    integrationKey: "n8n",
    ...
    fields: [
      { key: "webhook_url", label: "URL do Webhook n8n", placeholder: "https://sua-instancia.n8n.cloud/webhook/…", required: true },
      { key: "auth_header", label: "Header de auth (opcional)", placeholder: "Bearer xxx ou Basic xxx", type: "password" },
    ],
    tools: ["trigger-workflow", "send-event", "send-payload"],
  },
  {
    icon: Database,
    name: "Make (Integromat)",
    shortDesc: "Cenários visuais via webhook customizado.",
    longDesc: "Cole a URL do módulo 'Custom Webhook' do Make. Seus agentes acionam cenários complexos com routers, filtros e agregadores — ótimo pra fluxos que Zapier acha caros (Make cobra por operação, não por app).",
    category: "Automação",
    status: "available",
    apiStatus: "live",
    integrationKey: "make",
    popularity: 6,
    developer: "Make",
    developerUrl: "https://make.com",
    fields: [
      { key: "webhook_url", label: "URL do Custom Webhook", placeholder: "https://hook.eu2.make.com/…", required: true },
      { key: "secret", label: "Secret (opcional)", placeholder: "Validação HMAC", type: "password" },
    ],
    tools: ["trigger-scenario", "send-event", "send-payload"],
  },
  {
    icon: Code,

...
// ── Painéis avançados (disclosure "Painéis avançados", dobrado) ──────
      ...
      <ErrorBoundary>
        <Suspense fallback={<SectionLoader />}>
          <div className="space-y-5">
            {/* ─── ABOVE-THE-FOLD · 5 painéis essenciais ─────────────────── */}

            {/* 1. Departamento pendente · só renderiza se houver checkout aberto */}
            <PendingDepartmentCard />

            {/* 2. Hero · KPIs + próxima ação */}
            <HeroBriefing
            ...
            {/* 3. ROI do mês · prova de valor */}
            <MonthlyROICard onCTA={() => onSetActiveSection("omnix")} />

            {/* 4. Setup progressivo da conta */}
            <NextStepsCard />

            {/* 5. Primeiro entregável · auto-hide após 7 dias */}
            <FirstDeliveryCard onOpenApprovals={() => onSetActiveSection("approvals")} />

            {/* ─── DISCLOSURE 1 · Painéis operacionais ───────────────────── */}
            {agents.length > 0 && (
              <div className="pt-2">
                <button
                ...
                >
                  {showOperational ? (
                    <><ChevronUp className="h-3.5 w-3.5" /> Ocultar painéis operacionais</>
                  ) : (
                    <><ChevronDown className="h-3.5 w-3.5" /> Ver painéis operacionais (departamentos · tarefas · alertas)</>
                  )}
                </button>
                  ...
                  {showOperational && (
                    ...
                    >
                      <div className="space-y-5 pt-4">
                        ...
                        <ContractedDepartments onExplore={() => onSetActiveSection("library")} />
                        <TaskRequestPanel
            ...
            {/* ─── DISCLOSURE 2 · Painéis avançados ──────────────────────── */}
            {agents.length > 0 && (
              <div className="pt-1">
                <button
                ...
                >
                  {showAdvanced ? (
                    <><ChevronUp className="h-3.5 w-3.5" /> Ocultar painéis avançados</>
                  ) : (
                    <><ChevronDown className="h-3.5 w-3.5" /> Ver painéis avançados (ROI · integrações · indicações)</>
                  )}
                </button>
                      ...
                      <div className="space-y-5 pt-4">
                        ...
                        <div className="flex items-center justify-center pt-2">
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => onSetActiveSection("intelligence-hub")}
                            className="gap-1.5 text-xs text-muted-foreground hover:text-foreground"
                          >
                            Ver Intelligence Hub completo

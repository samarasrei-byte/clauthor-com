...
const ConciergeChat = ({ isOpen, onClose, onNavigate }: ConciergeChatProps) => {
  ...
  const sendToBackend = useCallback(async (apiMessages: ConciergeMessage[], isAutomatic = false) => {
    ...
    try {
      ...
      if (!token) {
        toast.error(t("support.login_required"));
        setIsLoading(false);
      ...
      if (!response.ok) {
        ...
        toast.error(data.error || t("support.concierge_error"));
        setIsLoading(false);
        setIsStreaming(false);
        return;
      }
      ...
      toast.error(t("support.connection_error_short"));
    } finally {
      setIsLoading(false);
      setIsStreaming(false);
        ...
        >
          ...
          <div className="p-4 border-b border-border flex items-center justify-between bg-gradient-to-r from-primary/5 to-transparent">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary/20 to-primary/10 flex items-center justify-center animate-pulse-glow">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-[300px]">
            ...
            {messages.map((msg, idx) => (
              ...
              >
                <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 ${
                  msg.role === "user" ? "bg-primary/20" : "bg-gradient-to-br from-primary/15 to-primary/10"
                }`}>
                  {msg.role === "user" ? (
                ...
                <div className={`max-w-[85%] rounded-xl px-3.5 py-2.5 ${
                  ...
                  {msg.role === "assistant" ? (
                    <div className="text-sm prose prose-sm dark:prose-invert max-w-none [&_p]:mb-1.5 [&_li]:mb-0.5">
            ...
            {isLoading && messages[messages.length - 1]?.role !== "assistant" && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-2.5">
                <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-primary/15 to-primary/10 flex items-center justify-center">
                  <Wand2 className="h-3.5 w-3.5 text-primary" />
                </div>
          ...
          <div className="p-3 border-t border-border">
            ...
            >
              ...
              <Button
                type="submit"
                size="icon"
                className="h-9 w-9 shrink-0 shadow-[0_0_16px_hsl(var(--primary)/0.2)]"

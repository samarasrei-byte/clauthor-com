            ...
            <tbody>
              {filtered.map((log: any) => (
                <tr key={log.id} className="border-b border-white/[0.05] hover:bg-accent/20">
                  ...
                  <td className="p-3"><Badge variant="secondary" className={log.status === "success" ? "bg-success/10 text-success" : log.status === "error" ? "bg-destructive/10 text-destructive" : "bg-primary/10 text-primary/80"}>{log.status}</Badge></td>

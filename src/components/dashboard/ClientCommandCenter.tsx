...
const ClientCommandCenter = ({
  ...
  const kpiCards = [
    { icon: Bot, label: "Agentes Ativos", value: activeAgents, spark: [1, 2, 2, 3, 3, activeAgents], color: "text-primary" },
    { icon: Zap, label: "Execuções", value: totalExecutions, spark: [100, 200, 350, 500, 800, totalExecutions || 0], color: "text-cyan-400" },
    { icon: CheckCircle, label: "Taxa Sucesso", value: successRate, suffix: "%", spark: [95, 96, 97, 97.5, 98, successRate], color: "text-emerald-500" },
    { icon: DollarSign, label: "Economia/mês", value: estimatedSavings, prefix: "R$ ", spark: [2000, 4000, 5000, 6000, 7000, estimatedSavings || 0], color: "text-cyan-400" },
    { icon: Coins, label: "Tokens Usados", value: totalTokensUsed, spark: [0, 100, 300, 500, 800, totalTokensUsed || 0], color: "text-primary" },
    ...
    { icon: Shield, label: "Uptime", value: 99.9, suffix: "%", spark: [99.5, 99.7, 99.8, 99.9, 99.9, 99.9], color: "text-emerald-500" },
    { icon: Flame, label: "Assinaturas", value: subscriptions.length, spark: [0, 1, 1, 2, 2, subscriptions.length], color: "text-primary" },
  ];

  const executionChartData = [
    { name: "Seg", exec: 12, success: 11 },
    { name: "Ter", exec: 18, success: 17 },
    { name: "Qua", exec: 25, success: 24 },
    { name: "Qui", exec: 22, success: 21 },
    { name: "Sex", exec: 30, success: 29 },
    { name: "Sáb", exec: 15, success: 15 },
    { name: "Dom", exec: 8, success: 8 },
  ];
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground">{activeAgents} agentes ativos • {successRate}% de sucesso • Uptime 99.9%</p>
            </div>
          </div>
      ...
      {credits && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-card rounded-2xl p-5 border border-white/[0.06]">
          ...
          <div className="flex justify-between mt-2">
            <span className="text-[10px] text-muted-foreground">{remainingCredits.toLocaleString("pt-BR")} restantes</span>
            <span className="text-[10px] text-muted-foreground">
              Reset: {credits.credits_reset_at ? new Date(credits.credits_reset_at).toLocaleDateString("pt-BR", { day: "2-digit", month: "short" }) : "—"}
            </span>
          </div>
          <TokenUpgradeDialog trigger={
            <Button size="sm" variant="outline" className="w-full mt-3 gap-1.5 border-primary/20 text-primary text-xs">
              <Coins className="h-3 w-3" /> Upgrade de tokens
            </Button>
          } />
        </motion.div>
      )}
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="lg:col-span-3 glass-card rounded-2xl p-5 border border-white/[0.06]">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium">Execuções da Semana</span>
            </div>
            <Badge variant="secondary" className="text-[10px]">7 dias</Badge>
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="lg:col-span-2 glass-card rounded-2xl p-5 border border-white/[0.06]">
          ...
          {tierDistribution.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center h-[140px] text-center">
              ...
              <p className="text-xs text-muted-foreground">Nenhum agente criado</p>
              <Link to="/library"><Button size="sm" className="mt-2 text-xs">Explorar</Button></Link>
            </div>
          )}
        </motion.div>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-card rounded-2xl p-5 border border-white/[0.06]">
        <div className="flex items-center justify-between mb-4">
          ...
          <Badge variant="secondary" className="text-[10px]">Últimas 10</Badge>
        </div>
        {recentLogs.length === 0 ? (

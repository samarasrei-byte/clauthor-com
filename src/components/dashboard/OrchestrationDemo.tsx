...
const DEMO_SCENARIO: DelegationStep[] = [
  {
    ...
    targetAgent: "Analista Financeiro",
    task: "Gerar relatório DRE do último trimestre com projeções",
    tool: "generate_report",
    toolLabel: "Relatório Gerado",
    result: "Relatório DRE Q4/2025 gerado com 4 seções: Receita (+23%), Custos (-8%), EBITDA (R$ 1.2M), Projeção Q1/2026.",
    subActions: [
      { tool: "analyze_data", label: "Análise de Tendência", success: true },
      { tool: "generate_report", label: "Relatório DRE", success: true },
    ],
    delayMs: 1200,
  },
  {
    ...
    phase: "pending",
    sourceAgent: "Orchestrator",
    targetAgent: "Gerente de Vendas",
    task: "Prospectar 10 leads qualificados no segmento SaaS B2B",
    tool: "search_leads",
    toolLabel: "Leads Encontrados",
    result: "12 leads qualificados encontrados. Top 3: TechCorp (score 92%), DataFlow (88%), CloudBase (85%). Pipeline estimado: R$ 340K.",
    subActions: [
      { tool: "search_leads", label: "Busca de Leads", success: true },
      { tool: "create_task", label: "Follow-up Criado", success: true },
      { tool: "send_email", label: "Outreach Enviado", success: true },
    ],
    delayMs: 1800,
  },
  {
    ...
    phase: "pending",
    sourceAgent: "Orchestrator",
    targetAgent: "Coordenador de Operações",
    task: "Agendar reunião de review semanal e criar tarefas de follow-up",
    tool: "schedule_meeting",
    toolLabel: "Reunião Agendada",
    result: "Reunião 'Review Semanal Q1' agendada para sex 28/02 às 14h. 3 tarefas de follow-up criadas com responsáveis definidos.",
    subActions: [
      { tool: "schedule_meeting", label: "Reunião Agendada", success: true },
      { tool: "create_task", label: "Tarefas Criadas (3x)", success: true },
    ],
    delayMs: 1400,
  },
];
...
const tierColors: Record<string, string> = {
  ...
  "Analista Financeiro": "from-emerald-500 to-emerald-600",
  "Gerente de Vendas": "from-amber-500 to-amber-600",
  "Coordenador de Operações": "from-cyan-500 to-cyan-600",
};
...
const OrchestrationDemo = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-card rounded-2xl p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-display text-xl font-bold">Demonstração A2A</h2>
              <p className="text-sm text-muted-foreground">
                Orchestrator delega tarefas para 3 agentes automaticamente
              </p>
            </div>
          </div>
          <div className="flex gap-2">
            {!isRunning ? (
              <Button onClick={runDemo} className="glow gap-2">
                ...
                {completedCount > 0 ? "Executar Novamente" : "Iniciar Demo"}
              </Button>
            ) : (
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            <span>{completedCount}/{steps.length} delegações</span>
            <span>{totalSubActions} sub-ações executadas</span>
      ...
      >
        <div className={`glass-card rounded-2xl px-6 py-4 flex items-center gap-3 border-2 transition-colors ${
          isRunning ? "border-primary/50 shadow-lg shadow-primary/10" : "border-border"
        }`}>
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">Coordenador Central A2A</p>
          </div>
          {isRunning && (
            <motion.div
              animate={{ rotate: 360 }}
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <Badge variant="secondary" className={`text-[10px] shrink-0 ${
                      ...
                      {step.phase === "complete" && <><CheckCircle className="h-3 w-3 mr-1" /> Completo</>}
                      {step.phase === "executing" && <><Loader2 className="h-3 w-3 mr-1 animate-spin" /> Executando</>}
                      {step.phase === "routing" && <><ArrowRight className="h-3 w-3 mr-1" /> Roteando</>}
                      {step.phase === "pending" && <><Clock className="h-3 w-3 mr-1" /> Aguardando</>}
                    </Badge>
                  </div>
        ...
        {completedCount === steps.length && !isRunning && (
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-display font-bold">Orquestração Concluída</h3>
                <p className="text-xs text-muted-foreground">
                  {steps.length} delegações • {totalSubActions} sub-ações • {(totalTime / 1000).toFixed(1)}s
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-background/50 rounded-xl p-3 text-center">
                ...
                <p className="text-[10px] text-muted-foreground">Agentes Ativados</p>
              </div>
              <div className="bg-background/50 rounded-xl p-3 text-center">
                <p className="font-display text-2xl font-bold text-primary">{totalSubActions}</p>
                <p className="text-[10px] text-muted-foreground">Ações Executadas</p>
              </div>
              <div className="bg-background/50 rounded-xl p-3 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-4 text-center">
              💡 Na produção, este fluxo usa IA real via <strong>delegate_to_agent</strong> com recursão até 3 níveis.
              Acesse o <strong>Chat do Agente</strong> para testar delegações reais.
            </p>
          </motion.div>
        )}
      </AnimatePresence>
